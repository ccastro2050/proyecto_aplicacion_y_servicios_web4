...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8035
EXPOSE 8035
...
| `EXPOSE 8035` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8035"]
    ...
    NAV -->|"localhost:8035"| APIFACTURAS
    ...
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15462"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11466"| SQLSERVER
```
...
docker network create proyecto_aplicacion_y_servicios_web4_default

# 2. sqlserver
docker run -d --name sqlserver --network proyecto_aplicacion_y_servicios_web4_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11466:1433 mcr.microsoft.com/mssql/server:2022-latest

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_aplicacion_y_servicios_web4_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init.sh

# 5. postgres
docker run -d --name postgres --network proyecto_aplicacion_y_servicios_web4_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_aplicacion_y_servicios_web4_default --restart unless-stopped `
  ...
  -e "ConnectionStrings__Postgres=Host=postgres;Port=5432;Database=bdfacturas_postgres_local;Username=postgres;Password=Paradigmas123!" `
  -e "Motor=${MOTOR_BD:-postgres}" `
  -v "${PWD}/api_facturas:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8035:8035 api-facturas
...
```yaml
URL_API: http://api-facturas:8035
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8035` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8035` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8035:8035`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8035`, el front
...
```powershell
docker network inspect proyecto_aplicacion_y_servicios_web4_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_aplicacion_y_servicios_web4` produce
la red `proyecto_aplicacion_y_servicios_web4_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_aplicacion_y_servicios_web4-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_aplicacion_y_servicios_web4_default has active endpoints (name:"proyecto_aplicacion_y_servicios_web4-api-facturas-1" …)
```
